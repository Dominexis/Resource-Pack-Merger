...
with open(os.path.join(path, "Resource Pack Merger Input.txt"), "r", encoding="utf-8") as file:
    rp_list = file.read().split("\n")



# Iterate through resource packs
rp_list_output = ["List of constituent packs:\n"]
for rp in rp_list:
            ...
            if not os.path.exists(os.path.join(dest_rp_subdir, file_name)):
                ...
                if rp_subdir == "minecraft\\models\\item":
                    ...
                    if "\"overrides\":[" in new_contents:
                        ...
                        # Read existing file
                        with open(os.path.join(dest_rp_subdir, file_name), "r", encoding="utf-8") as file:
